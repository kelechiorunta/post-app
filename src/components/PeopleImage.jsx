...
const PeopleImage = ({n}) => {
    ...
    let placeholder = `${process.env.REACT_APP_BASE_URL}/peoplePlaceholder/`//`./images/${user.email}-small.png`//'./placeholder.png'//`${process.env.REACT_APP_BASE_URL}/stream/picture`; // Replace this with a valid image path
  
    // Memoize the picture URL to avoid unnecessary re-renders
    let pictureUrl = useMemo(() => {
      return `${process.env.REACT_APP_BASE_URL}/peoplePicture/`;
    }, [`${process.env.REACT_APP_BASE_URL}/peoplePicture/`]); // Empty dependency array since it doesn't depend on props or state
  ...
  export default memo(PeopleImage);
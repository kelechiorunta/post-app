...
const ProfileImage = ({n}) => {
    const [isLoadingPic, setIsLoading] = useState(true);
  
    const user = JSON.parse(localStorage.getItem('userData'))
    console.log(user)
    // Placeholder image (use a local or remote placeholder URL)
    let placeholder = `${process.env.REACT_APP_BASE_URL}/profilePlaceholder/`//`./images/${user.email}-small.png`//'./placeholder.png'//`${process.env.REACT_APP_BASE_URL}/stream/picture`; // Replace this with a valid image path
  
    // Memoize the picture URL to avoid unnecessary re-renders
    let pictureUrl = useMemo(() => {
      return `${process.env.REACT_APP_BASE_URL}/profilePicture/`;
    }, [`${process.env.REACT_APP_BASE_URL}/profilePicture/`]); // Empty dependency array since it doesn't depend on props or state
